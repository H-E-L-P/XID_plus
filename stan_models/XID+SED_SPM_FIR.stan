...
data {
  ...
  //----MIPS----
  int<lower=0> npix_mips;//number of pixels
  int<lower=0> nnz_mips; //number of non neg entries in A
  vector[npix_mips] db_mips;//flattened map
  vector[npix_mips] sigma_mips;//flattened uncertianty map (assuming no covariance between pixels)
  real bkg_prior_mips;//prior estimate of background
  real bkg_prior_sig_mips;//sigma of prior estimate of background
  vector[nnz_mips] Val_mips;//non neg values in image matrix
  int Row_psw[nnz_mips];//Rows of non neg valies in image matrix
  int Col_psw[nnz_mips];//Cols of non neg values in image matrix
  vector[nsrc] f_low_lim_mips;//upper limit of flux
  vector[nsrc] f_up_lim_mips;//upper limit of flux
  //----PSW----
...
transformed data {
  ...
  for (s in 1:nSED){
	...
	tmp_sed[z,s,4] <- SEDs[z,s]/SEDs[z+3*nz,s];
      }}
}

parameters {
vector<lower=0.0,upper=1.0>[nsrc] src_f_mips;//source vector
  real bkg_mips;//background
...
model {
  vector[npix_mips] db_hat_mips;//model of map
  ...
  row_vector[4] f_vec[nsrc];//matrix of source fluxes
  ...
  for (n in 1:nsrc) {
    ...
    f_vec[n,4] <- f_low_lim_mips[n]+(f_up_lim_mips[n]-f_low_lim_mips[n])*src_f_mips[n];
    tmp_fvec[1] <- f_vec[n,3]/f_vec[n,1];
    tmp_fvec[2] <- f_vec[n,3]/f_vec[n,2];
    tmp_fvec[3] <- f_vec[n,2]/f_vec[n,1];
    tmp_fvec[3] <- f_vec[n,1]/f_vec[n,4];
    for (s in 1:nSED){
      for (z in 1:nz) {
  ...
  bkg_mips ~normal(bkg_prior_plw,bkg_prior_sig_mips);
  ...
  for (k in 1:npix_mips) {
    db_hat_mips[k] <- bkg_mips;
  }
  for (k in 1:nnz_psw) {
    db_hat_mips[Row_mips[k]+1] <- db_hat_mips[Row_mips[k]+1] + Val_mips[k]*f_vec[Col_mips[k]+1,1];
      }
  ...
  db_mips ~ normal(db_hat_mips,sigma_mips);
