...
for i in np.arange(0,len(tiles)):
    ...
    if ind.sum() >0:
	    ...
	    post_ind=np.append(np.append(np.arange(0,prior250.nsrc),prior250.nsrc+1+np.arange(0,prior250.nsrc)),2*prior250.nsrc+2+np.arange(0,prior250.nsrc))
	    posterior.stan_fit[:,:,post_ind]=np.power(10.0,posterior.stan_fit[:,:,post_ind])
