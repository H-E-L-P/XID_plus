...
\usepackage{hyperref}
\title[HELP:XID+]
  {HELP: XID+, The Probabilistic De-blender for \emph{Herschel} SPIRE maps}\author[P.D. Hurley et al.]{P.D.~Hurley,$^1$\thanks{Email: p.d.hurley@sussex.ac.uk} S.~Oliver,$^1$ J.~Scudder,$^1$ M.~Griffin,$^2$ S.~Duivenvoorden,$^1$ M.~Sargent$^1$ \\
...
%Infrared astronomy
Ever since the discovery of the far-infrared background with the \emph{Cosmic Background Explorer} COBE; \citep{Puget:1996}, surveys have aimed to observe and detect the sources responsible. Most of those sources are galaxies, with the far-infrared emission coming from dust. 
...
We adopt a simpler tiling scheme that splits the map data into square tiles, with a user defined tile length, $t_l$. The tiles overlap with each other by 25\% in both Right Ascension and Declination. This makes sure no source is less than 12.5\% of $t_l$ away from its optimum tile. So that we consider all sources that could foreseeably contribute to the map data in a tile, we fit to sources that are within 0.01 degrees (i.e. FWHM of SPIRE 500 $\mathrm{\mu m}$ beam) of the tile boundary. For our final combined posterior, for each source, we take the posterior from the optimum tile, i.e. the tile in which the source lies closest to the centre.
\begin{figure} 
...
The choice of $t_l$ effects the computational time of \textsc{XID+}. The required CPU time is found to scale linearly with the amount of data (i.e. number of pixels). Figure \ref{fig:prof} shows the CPU time as a function of tile area.
