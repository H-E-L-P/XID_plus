...
Our data ($\mathbf{d}$) are maps with $n_1 \times n_2 = M$ pixels. Our model assumes the maps are formed from $S$ known sources, with flux density $\mathbf{f}$ and a background term accounting for unknown sources. The point response function (PRF) tells us the contribution each source makes to each pixel in the map. Our map can therefore be described as follows:
...
As instrumental and confusion noise are independent, we can combine the two noise terms into one covariance matrix such that $\mathbf{N_d} = \Sigma_{instrumental}+\Sigma_{confusion}$. If we assume the only correlated noise between pixels is from the PSF acting on confusion noise, then diagonal entries of $\mathbf{N_d}$ are simply $\sigma_{inst.}^2+\sigma_{conf.}^2$), and off diagonal entries given by combining the PSF and $\sigma_{conf.}^2$.
