...
def create_XIDp_SPIREcat_nocov(posterior,prior250,prior350,prior500):
    ...
    c16 = fits.Column(name='Sig_conf_SPIRE_250', format='E',unit='mJy/Beam', array=np.full(nsrc,med_flux[(3*nsrc)+2]))
    c17 = fits.Column(name='Sig_conf_SPIRE_350', format='E',unit='mJy/Beam', array=np.full(nsrc,med_flux[(3*nsrc)+3]))
    c18 = fits.Column(name='Sig_conf_SPIRE_500', format='E',unit='mJy/Beam', array=np.full(nsrc,med_flux[(3*nsrc)+4]))
