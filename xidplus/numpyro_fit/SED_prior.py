...
def megacam_irac_spire_model_fix_agn(priors,sed_prior,agn,f_irac1,f_irac1_sig,f_opt,f_opt_sig,opt):
    pointing_matrices = [([p.amat_row, p.amat_col], p.amat_data) for p in priors]
    bands=np.array([opt,6,7,8,9,10])
    ...
    with numpyro.plate('n_param', 2):
        ...
        #agn_frac=numpyro.sample('agn_frac',dist.Normal(sed_prior.params_mu[:,1],sed_prior.params_sig[:,1]))
        #redshift=numpyro.sample('redshift',dist.Normal(sed_prior.params_mu[:,2],sed_prior.params_sig[:,2]))
    print(params.shape,agn.shape)
    params_tmp = jnp.hstack((params[:,0][:,None], agn[:,None], params[:,1][:,None]))

    src_f=jnp.power(10.0,sed_prior.emulator['net_apply'](sed_prior.emulator['params'],params_tmp))[:,bands]

    #src_f = numpyro.sample('src_f', dist.(flux_lower, flux_upper))
    db_hat_mips24 = sp_matmul(pointing_matrices[0], src_f[:, 2][:, None], priors[0].snpix).reshape(-1) + bkg[0]
    db_hat_psw = sp_matmul(pointing_matrices[1], src_f[:, 3][:, None], priors[1].snpix).reshape(-1) + bkg[1]
    db_hat_pmw = sp_matmul(pointing_matrices[2], src_f[:, 4][:, None], priors[2].snpix).reshape(-1) + bkg[2]
    db_hat_plw = sp_matmul(pointing_matrices[3], src_f[:, 5][:, None], priors[3].snpix).reshape(-1) + bkg[3]

    #sigma_tot_psw = jnp.sqrt(jnp.power(priors[0].snim, 2) + jnp.power(sigma_conf[0], 2))
    #sigma_tot_pmw = jnp.sqrt(jnp.power(priors[1].snim, 2) + jnp.power(sigma_conf[1], 2))
    #sigma_tot_plw = jnp.sqrt(jnp.power(priors[2].snim, 2) + jnp.power(sigma_conf[2], 2))
    with numpyro.plate('f_opt_sources',f_irac1_sig.size):
        numpyro.sample("obs_f_opt",dist.Normal(src_f[:,0],f_opt_sig),
                       obs=f_opt)
    with numpyro.plate('f_irac1_sources',f_irac1_sig.size):
        numpyro.sample("obs_f_irac1",dist.Normal(src_f[:,1],f_irac1_sig),
                       obs=f_irac1)
    with numpyro.plate('mips24_pixels', priors[0].snim.size):  # as ind_psw:
        numpyro.sample("obs_mips24", dist.Normal(db_hat_mips24,priors[0].snim ),
                       obs=priors[0].sim)
...
def all_bands_megacam_irac_fix_agn(priors,sed_prior,agn,f_irac1,f_irac1_sig,f_opt,f_opt_sig,opt,num_samples=500,num_warmup=500,num_chains=4,chain_method='parallel'):
    from numpyro.infer.reparam import LocScaleReparam
    from numpyro.handlers import reparam
    numpyro.set_host_device_count(os.cpu_count())
    reparam_model=reparam(megacam_irac_spire_model_fix_agn, config={'params': LocScaleReparam(0)})
    nuts_kernel = NUTS(reparam_model)
    mcmc = MCMC(nuts_kernel, num_samples=num_samples, num_warmup=num_warmup,num_chains=num_chains,chain_method=chain_method)
    rng_key = random.PRNGKey(0)
    mcmc.run(rng_key, priors,sed_prior,agn,f_irac1,f_irac1_sig,f_opt,f_opt_sig,opt,extra_fields=('potential_energy',))
    return mcmc

def megacam_irac_spire_model(priors, sed_prior, f_irac1, f_irac1_sig, f_opt, f_opt_sig, opt):
    pointing_matrices = [([p.amat_row, p.amat_col], p.amat_data) for p in priors]
    fit_bands = np.array([opt, 6, 7, 8, 9, 10])
    ...
    with numpyro.plate('n_param', 3):
        with numpyro.plate('nsrc', priors[0].nsrc):
            with numpyro.handlers.reparam(config={"params": TransformReparam()}):
                params = numpyro.sample('params', dist.TransformedDistribution(dist.Normal(0.0, 1.0),
                                                                               dist.transforms.AffineTransform(
                                                                                   sed_prior.params_mu,
                                                                                   sed_prior.params_sig)))
        # sfr=numpyro.sample('sfr',dist.Normal(sed_prior.params_mu[:,0],sed_prior.params_sig[:,0]))
        # agn_frac=numpyro.sample('agn_frac',dist.Normal(sed_prior.params_mu[:,1],sed_prior.params_sig[:,1]))
        # redshift=numpyro.sample('redshift',dist.Normal(sed_prior.params_mu[:,2],sed_prior.params_sig[:,2]))
    #params_tmp = jnp.hstack((params[:, 0][:, None], agn[:, None], params[:, 1][:, None]))

    src_f= jnp.power(10.0, sed_prior.emulator['net_apply'](sed_prior.emulator['params'], params))[..., fit_bands]

    # src_f = numpyro.sample('src_f', dist.(flux_lower, flux_upper))
    db_hat_mips24 = sp_matmul(pointing_matrices[0], src_f[..., [2]], priors[0].snpix).reshape(-1) + bkg[...,0]
    db_hat_psw = sp_matmul(pointing_matrices[1], src_f[..., [3]], priors[1].snpix).reshape(-1) + bkg[...,1]
    db_hat_pmw = sp_matmul(pointing_matrices[2], src_f[..., [4]], priors[2].snpix).reshape(-1) + bkg[...,2]
    db_hat_plw = sp_matmul(pointing_matrices[3], src_f[..., [5]], priors[3].snpix).reshape(-1) + bkg[...,3]
...
def megacam_irac_spire_model_cholesky(priors, sed_prior, f_irac1, f_irac1_sig, f_opt, f_opt_sig, opt):
    pointing_matrices = [([p.amat_row, p.amat_col], p.amat_data) for p in priors]
    fit_bands = np.array([opt, 6, 7, 8, 9, 10])
    n_param=3
    bkg_mu = np.asarray([p.bkg[0] for p in priors]).T
    ...
    # params_sig=sed_prior.params_sig
    # params_sig[:,0]=lir_sig
    L_Rho = numpyro.sample('L_Rho', dist.LKJCholesky(3, concentration=0.05))

    def transform(params,params_mu, params_sig):
    ...
    src_f= jnp.power(10.0, sed_prior.emulator['net_apply'](sed_prior.emulator['params'], jax.vmap(transform)(params,sed_prior.params_mu,sed_prior.params_sig)))[..., fit_bands]
    # src_f = numpyro.sample('src_f', dist.(flux_lower, flux_upper))
    db_hat_mips24 = sp_matmul(pointing_matrices[0], src_f[..., [2]], priors[0].snpix).reshape(-1) + bkg[...,0]
    db_hat_psw = sp_matmul(pointing_matrices[1], src_f[..., [3]], priors[1].snpix).reshape(-1) + bkg[...,1]
    db_hat_pmw = sp_matmul(pointing_matrices[2], src_f[..., [4]], priors[2].snpix).reshape(-1) + bkg[...,2]
