...
def all_bands_SPM_prior(SPIRE_250,SPIRE_350,SPIRE_500,redshifts,SEDs,chains=4,iter=1000,optimise=False):
    """Fit all three SPIRE maps using stan"""
    ...
    print SEDs.shape
    #see if model has already been compiled. If not, compile and save it
    model_file=output_dir+"/SED_SPM.stan.pkl"
    try:
       with open(model_file,'rb') as f:
            # using the same model as before
            print("%s found. Reusing" % model_file)
       ...
       fit = sm.sampling(data=XID_data,iter=iter,chains=chains,verbose=True)
        ...
        print("%s not found. Compiling" % model_file)
        sm = pystan.StanModel(file=stan_path+'SED_SPM.stan')
        # save it to the file 'model.pkl' for later use
        with open(model_file, 'wb') as f:
            pickle.dump(sm, f)
...
def all_bands_SPM(SPIRE_250,SPIRE_350,SPIRE_500,redshifts,SEDs,chains=4,iter=1000,optimise=False):
    ...
    try:
       ...
       fit = sm.sampling(data=XID_data,iter=iter,chains=chains,verbose=True)
        ...
        print("%s not found. Compiling" % model_file)
        sm = pystan.StanModel(file=stan_path+'XID+SED_SPM.stan')
        # save it to the file 'model.pkl' for later use
        with open(model_file, 'wb') as f:
            pickle.dump(sm, f)
...
def all_bands_SPM_ALMA(SPIRE_250,SPIRE_350,SPIRE_500,redshifts,SEDs,ALMA_flux,ALMA_sig,chains=4,iter=1000,optimise=False):
    """Fit all three SPIRE maps using stan"""
    ...
    #see if model has already been compiled. If not, compile and save it
    model_file=output_dir+"/XID+SED_SPM_ALMA.stan.pkl"
    try:
       with open(model_file,'rb') as f:
            # using the same model as before
            print("%s found. Reusing" % model_file)
       ...
       fit = sm.sampling(data=XID_data,iter=iter,chains=chains,verbose=True)
        ...
        print("%s not found. Compiling" % model_file)
        sm = pystan.StanModel(file=stan_path+'XID+SED_SPM_ALMA.stan')
        # save it to the file 'model.pkl' for later use
        with open(model_file, 'wb') as f:
            pickle.dump(sm, f)
...
def SED_FIR(MIPS_24,SPIRE_250,SPIRE_350,SPIRE_500,redshifts,SEDs,chains=4,iter=1000,optimise=False):
    """Fit all three SPIRE maps using stan"""
    ...
    XID_data={'nsrc':SPIRE_250.nsrc,
          ...
          'f_low_lim_mips':MIPS_24.prior_flux_lower,
          'f_up_lim_mips':MIPS_24.prior_flux_upper,
          'npix_psw':SPIRE_250.snpix,
          'nnz_psw':SPIRE_250.amat_data.size,
          'db_psw':SPIRE_250.sim,
          'sigma_psw':SPIRE_250.snim,
    ...
    #see if model has already been compiled. If not, compile and save it
    model_file=output_dir+"/XID+SED_SPM_FIR.stan.pkl"
    try:
       with open(model_file,'rb') as f:
            # using the same model as before
            print("%s found. Reusing" % model_file)
       ...
       fit = sm.sampling(data=XID_data,iter=iter,chains=chains,verbose=True)
        ...
        print("%s not found. Compiling" % model_file)
        sm = pystan.StanModel(file=stan_path+'XID+SED_SPM_FIR.stan')
        # save it to the file 'model.pkl' for later use
        with open(model_file, 'wb') as f:
            pickle.dump(sm, f)
